...
class VerboseTranscriptionResponse(BaseModel):
    task: Optional[str] = None  # Made optional for Galleria API compatibility
    language: str
    duration: float
    text: str
...
# Galleria API (LocalWhisper) response format
class LocalWhisperWord(BaseModel):
    start: float
    end: float
    word: str
    probability: float

class LocalWhisperSegment(BaseModel):
    start: float
    end: float
    text: str
    words: List[LocalWhisperWord]

class LocalWhisperVerboseTranscriptionResponse(BaseModel):
    round_id: int
    video_id: str
    status: str
    text: str
    ...
    segments: List[LocalWhisperSegment]

    model_config = {
        "extra": "ignore"
    }

def convert_localwhisper_to_openaiapi(local_response: LocalWhisperVerboseTranscriptionResponse) -> VerboseTranscriptionResponse:
    """
    Convert LocalWhisper (Galleria API) response format to standard VerboseTranscriptionResponse format.
    Extracts all words from segments and flattens them into a single words list.
    """
    all_words = []
    for segment in local_response.segments:
    ...
    return VerboseTranscriptionResponse(
        task="transcribe",  # Default value
        language=local_response.language,
        duration=local_response.duration,
...
@router.post("/transcribe-background/{round_id}")
async def transcribe_audio_background(
    round_id: int,
    db: AsyncSession = Depends(get_db)
):
    """
    Step 1-B (Background): Start transcription via Galleria API background task.
    - Input: round_id (int)
    - Output: Returns immediately with task_id
    - Note: Transcription runs in background. Use /job-progress to check status.
    """
    start_time = time.time()
    print(f"[Step 1-B Background: /transcribe-background] 処理開始 - round_id: {round_id}")
    ...
    # Start background transcription via Galleria API
    try:
        from services.transcription_service import start_transcription_background
        
        # Construct YouTube URL from video_id
        youtube_url = f"https://www.youtube.com/watch?v={video_id}"
        
        print(f"Starting background transcription for round_id: {round_id}, video_id: {video_id}")
        result = await start_transcription_background(
            round_id=round_id,
            url=youtube_url,
            num_chunks=4,
            max_workers=2,
        ...
        # Debug: Print the actual response from Galleria API
        print(f"[DEBUG] Galleria API response: {result}")
        
        task_id = result.get("task_id")
        # task_idは必須ではない可能性があるので、警告のみ出す
        if not task_id:
            logger.warning(f"No task_id in response from Galleria API. Full response: {result}")
        else:
            logger.info(f"Step 1-B Background: Transcription task started for Round {round_id}, task_id={task_id}")

        elapsed_time = time.time() - start_time
        print(f"[Step 1-B Background: /transcribe-background] タスク開始完了 - 処理時間: {elapsed_time:.2f}秒")

        return {
            "status": result.get("status", "started"),
            "round_id": round_id,
            "task_id": task_id,  # None でも OK
            "message": result.get("message", "Transcription started in background. Use /job-progress to check status."),
            "processing_time_seconds": round(elapsed_time, 2),
            "galleria_response": result  # デバッグ用に元のレスポンスも含める
        }
        ...
        elapsed_time = time.time() - start_time
        print(f"[Step 1-B Background: /transcribe-background] Error: {str(e)}")
        logger.error(f"Error starting background transcription: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))


@router.post("/fetch-transcription-result/{round_id}")
async def fetch_transcription_result(
    round_id: int,
    db: AsyncSession = Depends(get_db)
):
    """
    Fetch completed transcription result from Galleria API and save to database.
    - Input: round_id (int)
    - Output: Saves transcription result to Round.raw_transcription
    - Note: Call this after background transcription is complete (status = COMPLETED)
    """
    start_time = time.time()
    print(f"[Fetch Transcription Result] 処理開始 - round_id: {round_id}")
    ...
    # Get transcription result from Galleria API
    try:
        from services.transcription_service import get_transcription_result
        
        print(f"Fetching transcription result from Galleria API for round_id: {round_id}")
        transcription_result = await get_transcription_result(round_id)
        
        # Validate and convert Galleria API response format
        try:
            # First, validate as LocalWhisper format
            local_whisper_data = LocalWhisperVerboseTranscriptionResponse(**transcription_result)
            logger.info(f"Galleria API response validated as LocalWhisper format for round {round_id}")
            
            # Convert to standard VerboseTranscriptionResponse format
            standard_format = convert_localwhisper_to_openaiapi(local_whisper_data)
            logger.info(f"Converted to standard format: {len(standard_format.words)} words extracted")
            
            # Convert back to dict for storage
            transcription_dict = standard_format.model_dump()
            
        except ValidationError as val_error:
            logger.error(f"Validation failed for Galleria API response: {val_error}")
            raise HTTPException(status_code=500, detail=f"Invalid transcription format from Galleria API: {val_error}")

        # Save converted transcription to Round
        round_obj.raw_transcription = transcription_dict
        await db.commit()
        
        logger.info(f"Transcription result saved to DB for Round {round_id}")

        elapsed_time = time.time() - start_time
        print(f"[Fetch Transcription Result] 処理完了 - 処理時間: {elapsed_time:.2f}秒")

        return {
            "status": "success",
            "round_id": round_id,
            "message": "Transcription result fetched and saved to database",
            "words_count": len(standard_format.words),
            "processing_time_seconds": round(elapsed_time, 2)
        }
        ...
        elapsed_time = time.time() - start_time
        print(f"[Fetch Transcription Result] Error: {str(e)}")
        logger.error(f"Error fetching transcription result: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))
