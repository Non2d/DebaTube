    ...
    <div className="text-center mb-8">
      ...
      {isRecording && (
        <div className="mt-4 flex justify-center items-center">
          <div className={`animate-pulse rounded-full w-4 h-4 mr-3 ${isOverTime ? 'bg-red-600' : 'bg-red-500'
            }`}></div>
          <span className={`text-xl font-medium ${isOverTime ? 'text-red-600' : 'text-red-500'
            }`}>
            {isOverTime ? t('timer.overTime') : t('timer.recording')}
          </span>
        </div>
      )}
    </div>
